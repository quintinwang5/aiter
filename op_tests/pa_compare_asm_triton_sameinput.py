...
"""Same-input numerical comparison: SP3 asm `pa_decode_bf16_asm` vs Triton `unified_attention`.

Unlike op_tests/compare_pa_decode_asm_vs_triton.py (which runs the two bench scripts as
separate processes -> DIFFERENT random inputs, perf-only), this drives BOTH kernels on the
SAME logical Q/K/V/block_tables/scales/sink, generated ONCE via the upstream triton
`generate_data`, each fed through its own memory layout:
...
Per config it prints 4 comparisons (RMS-relative error + max abs):
  triton_vs_ref : unified_attention   vs ref_paged_attn   (triton's own torch ref)
  asm_vs_ref    : pa_decode_bf16_asm  vs ref_pa_decode     (asm's own torch ref)
  asm_vs_triton : the cross-kernel comparison               <-- the point of this test
  ref_vs_ref    : ref_paged_attn      vs ref_pa_decode      (sanity: refs agree on same data)

  python op_tests/compare_asm_triton_sameinput.py -b 1 -kvh 8 -c 1024 -m 0
  python op_tests/compare_asm_triton_sameinput.py -b 1 8 -kvh 8 -c 256 1024 4097 -m 0 --scales 1.0 1.0 1.0
"""
...
def rms_rel(ref: torch.Tensor, out: torch.Tensor) -> float:
    """RMS(out-ref) / RMS(ref) in fp32 -- the kernel's actual acceptance metric."""
    ref = ref.float()
    out = out.float()
...
def max_abs(ref: torch.Tensor, out: torch.Tensor) -> float:
...
def compare_one(
    ...
    # ---- reshape everything to [batch, qlen, Hkv, gqa, D] fp32 and compare ----
    tri = out_tri.reshape(batch, qlen, kv_head_num, gqa, head_dim)
    tri_ref = ref_tri.reshape(batch, qlen, kv_head_num, gqa, head_dim)
    asm = out_asm.reshape(batch, qlen, kv_head_num, gqa, head_dim)
    asm_ref = ref_asm.reshape(batch, qlen, kv_head_num, gqa, head_dim)
    ...
    return {
        ...
        "triton_vs_ref": rms_rel(tri_ref, tri),
        "asm_vs_ref": rms_rel(asm_ref, asm),
        "asm_vs_triton": rms_rel(tri, asm),
        "ref_vs_ref": rms_rel(tri_ref, asm_ref),
        "max|asm-tri|": max_abs(tri, asm),
    }
...
def main():
    ...
    p.add_argument(
        ...
        type=float,
        default=5e-2,
        help="asm_vs_triton RMS-rel threshold for the PASS/FAIL column (default 0.05)",
    )
    a = p.parse_args()
    ...
    if a.context_lens is not None:
        ...
        for b, kvh, c, mtp in itertools.product(
            ...
            rows.append(
                compare_one(b, kvh, c, mtp, a.scales, a.varlen, a.sink, None)
            )

    df = pd.DataFrame(rows)
    df["PASS"] = df["asm_vs_triton"] <= a.tol
    fmt = {
        c: "{:.2e}".format
        for c in ["triton_vs_ref", "asm_vs_ref", "asm_vs_triton", "ref_vs_ref", "max|asm-tri|"]
    }
    print(df.to_string(index=False, formatters=fmt))
    df.to_csv("compare_asm_triton_sameinput.csv", index=False)
    n_fail = int((~df["PASS"]).sum())
    print(
        f"\nasm_vs_triton PASS {len(df) - n_fail}/{len(df)} (tol={a.tol}). "
        "triton_vs_ref / asm_vs_ref isolate a per-kernel layout bug; "
        "ref_vs_ref confirms both torch refs agree on the identical inputs."
    )
