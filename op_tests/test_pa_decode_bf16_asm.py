...
Metadata is generated by aiter.get_pa_metadata_v1() (GPU kernel, same as ATOM/production).
Split-KV partial results are merged by cpu_reduce_v1(), a pure-Python implementation that
is logically identical to aiter.pa_reduce_v1() (mla_reduce_v1 kernel).  The legacy
make_sched2_metadata / cpu_reduce are kept for reference only.
"""
...
def make_sched2_metadata(
    ...
    """[LEGACY — use build_pa_metadata() instead for production use.]

    Python port of sched2 common_ps.h generate_metadata, kept for reference.
    Generates metadata at page granularity (1 page per unit), which produces
    kv_head_num work items per TG and creates unnecessary splits for short
    contexts.  build_pa_metadata() delegates to aiter.get_pa_metadata_v1()
...
def cpu_reduce_v1(out, split_o, split_lse, reduce_indptr, reduce_final_map,
    ...
    for g in range(len(rip) - 1):
        ...
        # emu index split_o as `partial_o_loc + qi` directly — do NOT multiply by qlen
        # again. The old `* qlen_with_mtp` double-scaled it (correct only for mtp=0 where
        # qlen=1) -> wrong/zero rows for mtp>=1 at deep split. (Matches emu/reduce.cu now.)
...
def test_pa_decode(
    ...
    # CPU reduce: merge split partials (no-op when all work items are direct-to-O,
    # which is the common case with get_pa_metadata_v1 fast_mode=True).
    cpu_reduce_v1(out, split_o, split_lse, reduce_indptr, reduce_final_map,
                  reduce_partial_map, qlen_with_mtp)
...
def test_pa_decode_vmask(
    ...
    def rd(o, so, sl):
        ql = inp.get("qlen_with_mtp", 1)
        return cpu_reduce_v1(
            o.clone(), so, sl,
            inp["reduce_indptr"], inp["reduce_final_map"], inp["reduce_partial_map"],
            ql,
        )
