...
and split-KV partials are merged by aiter.pa_reduce_v1() (GPU kernel).  The legacy Python
...
def test_pa_decode(
    ...
    # GPU reduce: merge split partials into out (no-op when all work items are
    # direct-to-O, which is the common case with get_pa_metadata_v1 fast_mode).
    aiter.pa_reduce_v1(
        split_o,
        split_lse,
        reduce_indptr,
        ...
        qlen_with_mtp,
        0,
        out.reshape(batch * qlen_with_mtp, q_head_num, head_dim),
    )
    torch.cuda.synchronize()
...
def test_pa_decode_vmask(
    ...
    def rd(o, so, sl):
        out = o.clone()
        b, ql = inp["batch"], inp["qlen_with_mtp"] if "qlen_with_mtp" in inp else 1
        aiter.pa_reduce_v1(
            so, sl,
            inp["reduce_indptr"], inp["reduce_final_map"], inp["reduce_partial_map"],
            ql, 0,
            out.reshape(b * ql, inp["q_head_num"], inp["head_dim"]),
        )
        torch.cuda.synchronize()
        return out
